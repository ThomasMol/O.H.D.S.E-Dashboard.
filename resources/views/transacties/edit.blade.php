...
@section('content')
    <h2 class="mb-4">Transactie wijzigen</h2>
...
<form class="card" method="POST" action="/transacties/{{$transactie->transactie_id}}">
        ...
        <div class="form-row">
            <div class="col-md">
                ...
                <input type="date" class="form-control mb-3" id="datum" name="datum" value="{{ $transactie->datum }}" required>
            </div>
            <div class="col-md">
            ...
            <div class="col-md">
                ...
                <select class="form-control mb-3" id="mutatie_soort" name="mutatie_soort" required>
                    ...
                    @foreach($transactie->mutatieOptions() as $key => $mutatie)
                        <option {{$transactie->mutatie_soort == $key ? 'selected' : ''}} value="{{ $key }}" >{{ $mutatie }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="number" class="form-control" id="bedrag" name="bedrag" step=".01" value="{{ $transactie->bedrag }}" min="0" max="99999999" placeholder="0.00" required>
                </div>
            </div>
        </div>
            ...
            <div class="col-md">
                ...
                <select class="form-control mb-3"id="lid_id" name="lid_id">
                    ...
                    @foreach($leden as $lid)
                        <option {{$transactie->lid_id == $lid->lid_id ? 'selected' : ''}} value="{{$lid->lid_id}}">{{$lid->roepnaam}} {{$lid->achternaam}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md">
                ...
                <select class="form-control mb-3"id="spaarplan" name="spaarplan">
                    @foreach($transactie->spaarplanOptions() as $key => $spaarplan)
                        <option {{$transactie->spaarplan === $key ? 'selected' : ''}} value="{{ $key }}" >{{ $spaarplan }}</option>
                    @endforeach
                </select>
        ...
        <input type="text" class="form-control mb-3" id="tegenrekening" name="tegenrekening" value="{{ $transactie->tegenrekening }}" >

        <label for="mededelingen">Mededelingen</label>
        <input type="text" class="form-control mb-3" id="mededelingen" name="mededelingen" value="{{ $transactie->mededelingen }}" required>
