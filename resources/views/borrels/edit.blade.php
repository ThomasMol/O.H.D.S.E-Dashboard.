...
<form class="card" method="POST" action="/borrels/{{$borrel->borrel_id}}">
        ...
        <div class="col">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" class="form-control" id="budget" name="budget" step=".01"
                    value="{{ $borrel->budget }}" min="0" max="99999999" required>
            </div>
        </div>
        <div class="col">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" class="form-control " id="uitgave" name="uitgave" step=".01"
                    value="{{ $borrel->uitgave }}" min="0" max="99999999" required>
            </div>
        </div>
        <div class="col">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" class="form-control" id="naheffing" name="naheffing" step=".01"
                    value="{{ $borrel->naheffing }}" max="99999999" required readonly>
            </div>
        </div>
    </div>

    <label for="omschrijving">Omschrijving of opmerking</label>
    <textarea type="text" class="form-control mb-3" id="omschrijving"
        name="omschrijving">{{ $borrel->omschrijving }}</textarea>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><input class="form-control" type="checkbox" name="aanwezigheid[{{$lid->lid_id}}][aanwezig]"
                            value="1" @if(isset($lid->aanwezigh)) checked @endif></td>
                    <td><input class="form-control" id="naheffing_leden" type="checkbox"
                            name="aanwezigheid[{{$lid->lid_id}}][naheffing]" value="1" @if(isset($lid->naheffing))
                        checked @endif></td>
                    @if($lid->type_lid == "Actief")
                    <td><input class="form-control" type="checkbox" name="aanwezigheid[{{$lid->lid_id}}][afgemeld]"
                            value="1" @if(isset($lid->afgemeld)) checked @endif></td>
                    <td><input class="form-control" type="checkbox" name="aanwezigheid[{{$lid->lid_id}}][boete]"
                            value="1" @if(isset($lid->boete)) checked @endif></td>
                    @endif
                </tr>
                @endforeach
            </tbody>
    ...
    <input class="form-control" type="number" id="naheffing_leden_aantal" name="naheffing_leden_aantal" value="0"
        readonly hidden>
